...
export default async function TeamDashboardPage({ searchParams }: PageProps) {
  ...
  // Filter out group registrations from the direct list
  const nonGroupRegistrations = teamRegistrations.filter((reg) => {
    const prog = programMap.get(reg.programId);
    return prog?.section !== "group";
  });

  const teamResultItems: TeamResultItem[] = nonGroupRegistrations.map((reg, idx) => {
    const prog = programMap.get(reg.programId);
  ...
  // Group group registrations by programId
  const groupRegsByProgram: { [programId: string]: typeof teamRegistrations } = {};
  teamRegistrations.forEach((reg) => {
    const prog = programMap.get(reg.programId);
    if (prog?.section === "group") {
      if (!groupRegsByProgram[reg.programId]) {
        groupRegsByProgram[reg.programId] = [];
      }
      groupRegsByProgram[reg.programId].push(reg);
    }
  });

  // Process and push group programs as single summary cards with group members info
  Object.entries(groupRegsByProgram).forEach(([programId, regs]) => {
    const prog = programMap.get(programId);
    const approvedResult = approvedResultMap.get(programId);
    ...
    if (approvedResult) {
      const entry = approvedResult.entries.find((e) => e.team_id === team.id);
      if (entry) {
        score = entry.score;
        if (entry.position >= 1 && entry.position <= 3) {
          position = entry.position as 1 | 2 | 3;
    ...
    const groupMembers = regs.map((r) => ({
      name: r.studentName,
      chest: r.studentChest,
    }));

    teamResultItems.push({
      id: `group-summary-${programId}`,
      registrationId: regs[0]?.id || `group-reg-${programId}`,
      studentName: `${team.teamName} (Group Event)`,
      studentChest: "",
      programId,
      programName: prog?.name || regs[0]?.programName || "Group Program",
      category: prog?.category || "GENERAL",
      section: "group",
      status,
      position,
  ...
  // Also include group results assigned directly to team.id that aren't already covered by registrations
    ...
    if (prog?.section === "group") {
      res.entries.forEach((entry, idx) => {
        if (entry.team_id === team.id && !entry.student_id) {
          const exists = teamResultItems.some((i) => i.programId === res.program_id);
          if (!exists) {
